...
abstract final class AppGlassTokens {
  ...
  static double customOpacity(AppGlassLevel level, Brightness brightness) {
    ...
    return switch (level) {
      AppGlassLevel.compact => dark ? 0.40 : 0.50,
      AppGlassLevel.surface => dark ? 0.46 : 0.56,
      AppGlassLevel.dock => dark ? 0.42 : 0.52,
      AppGlassLevel.sheet => dark ? 0.56 : 0.60,
    };
  }
}
...
class AppGlassSurface extends StatelessWidget {
  const AppGlassSurface({
    ...
    this.borderColor,
    this.tint,
    this.customBackgroundBlurSigma,
    this.customBackgroundOpacity,
    this.grainOpacity = 0,
    this.rendering = AppGlassRendering.auto,
    this.level = AppGlassLevel.surface,
  ...
  final Color? tint;
  final double? customBackgroundBlurSigma;
  ...
  Widget build(BuildContext context) {
    ...
    final dark = brightness == Brightness.dark;
    final customBackground = FlareBackgroundScope.hasCustomBackgroundOf(
      context,
    );
    ...
    final resolvedOpacity = customBackground
        ? customBackgroundOpacity ??
              AppGlassTokens.customOpacity(level, brightness)
        : (!useBlur && rendering == AppGlassRendering.auto ? 0.96 : opacity);
    final resolvedBlur = customBackground
        ? customBackgroundBlurSigma ?? AppGlassTokens.customBlur(level)
        : blurSigma;
        ...
        color:
            ...
            (dark
                ? Colors.white.withValues(
                    alpha: customBackground
                        ? 0.12
                        : AppGlassTokens.borderDarkOpacity,
                  )
                : Colors.black.withValues(
                    alpha: customBackground
                        ? 0.10
                        : AppGlassTokens.borderLightOpacity,
                  )),
      ),
      gradient:
          useBlur || customBackground || palette.glassTint != Colors.transparent
          ? LinearGradient(
...
final class FlareGlassSurface extends AppGlassSurface {
  const FlareGlassSurface({
    ...
    super.borderColor,
    super.tint,
    super.customBackgroundBlurSigma,
    super.customBackgroundOpacity,
    super.grainOpacity,
    super.rendering,
    super.level,
    super.padding,
...
class AppGlassCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final atmospheric =
        palette.glassTint != Colors.transparent ||
        FlareBackgroundScope.hasCustomBackgroundOf(context);
    final surface = AppGlassSurface(
      borderRadius: BorderRadius.circular(FlareRadii.large),
      backgroundColor: palette.surface,
      opacity: atmospheric ? (dark ? 0.70 : 0.78) : (dark ? 0.90 : 0.96),
    ...
    return onTap == null ? content : Semantics(button: true, child: content);
  }
}
      ...
      child: GestureDetector(
        ...
        child: AnimatedScale(
          ...
          child: FlareGlassSurface(
            ...
            backgroundColor: widget.accent ? palette.accent : palette.surface,
            opacity: widget.accent ? 0.11 : 0.68,
            customBackgroundOpacity: widget.accent ? 0.18 : null,
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      ...
      children: <Widget>[
        ...
        AppGlassSurface(
          borderRadius: BorderRadius.circular(FlareRadii.input),
          level: AppGlassLevel.compact,
