...
func TestHealthContracts(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			server := New(config.Config{HTTPAddress: ":0"}, "test", logger, test.ping, nil, nil)
...
func TestPublicPageIsUsefulAndSanitized(t *testing.T) {
	logger := slog.New(slog.NewTextHandler(io.Discard, nil))
	server := New(config.Config{HTTPAddress: ":0"}, "1.4.2", logger, pingFunc(func(context.Context) error { return nil }), nil, nil)
	response := httptest.NewRecorder()
...
func TestPublicPageReportsUnavailableDatabase(t *testing.T) {
	logger := slog.New(slog.NewTextHandler(io.Discard, nil))
	server := New(config.Config{HTTPAddress: ":0"}, "test", logger, pingFunc(func(context.Context) error { return errors.New("offline") }), nil, nil)
	response := httptest.NewRecorder()
	server.Handler.ServeHTTP(response, httptest.NewRequest(http.MethodGet, "/", nil))
